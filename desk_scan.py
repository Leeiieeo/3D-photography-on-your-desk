...
parser.add_argument("--light_source",type=str,default="./run/light/light_source_2.npy")
...
for frame_index in tqdm(range(total_frame)):
    ...
    # v = gray.astype(np.uint8)
    # v[row_top,int(np.floor(spatial_shadow[frame_index,0]))] = 255
    # v[row_down, int(np.floor(spatial_shadow[frame_index, 1]))] = 255
...
np.savetxt(args.saving + "/point_3.csv", point, delimiter=',')
